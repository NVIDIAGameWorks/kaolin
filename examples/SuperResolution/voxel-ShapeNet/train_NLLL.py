...
#Dataset
train_set = kal.datasets.ShapeNet.Voxels(root='../../datasets/', categories=args.categories,
                                         download=True, train=True, resolutions=[128, 32],
                                         split=.97)
...
class_weights = torch.from_numpy(np.asarray([0.0283, 1-.0283])).float().to(args.device)
