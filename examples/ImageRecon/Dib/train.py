...
point_set = kal.datasets.ShapeNet_Points(root='../../datasets/', cache_dir='cache/', categories=args.categories,
                                         train=True, split=.7, num_points=3000)
...
class Engine(object):
    ...
    def train(self):
        ...
        for i, data in enumerate(tqdm(dataloader_train), 0):
            ...
            # render image
            image_pred, alpha_pred, face_norms = renderer.forward(
                points=[(pred_verts * .57), mesh.faces], colors=[colours])
