...
meshes_set_valid = shapenet.ShapeNet_Meshes(root=args.shapenet_root, categories=args.categories,
                                            train=False, split=.7)
valid_set = shapenet.ShapeNet_Combination([points_set_valid, meshes_set_valid])
...
# Load saved weights
print(f"LOADING {os.path.join(logdir, 'recent.ckpt')}")
...
with torch.no_grad():
    for sample in tqdm(valid_set):
        ...
        # Inference
        pts_features = encoder(tgt_points.unsqueeze(0))
        generated_mesh, generated_verts, generated_faces = merge_mesh(decoder.generate_mesh(pts_features), args.device)
        ...
        if not args.no_vis:
            ...
            print(f'Rendering Predicted Mesh. Chamfer:{loss}')
