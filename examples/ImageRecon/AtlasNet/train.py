...
parser.add_argument('--cache-dir', type=str, default='cache', help='Path to intermediary assets.')
parser.add_argument('--expid-decoder', type=str, default=None, help='Path to get trained decoder from intermediate representation to.')
...
class Engine(object):
    ...
    def __init__(self, print_every, resume_name=None, resume_decoder=None):
        self.cur_epoch = 0
        self.train_loss = {}
        self.val_score = {}
        self.bestval = 0
        ...
        print("tt", resume_name, resume_decoder)
        if resume_decoder:
            self.load_decoder(resume_decoder)
    ...
    def train(self):
        ...
        for i, sample in enumerate(tqdm(dataloader_train), 0):
            ...
            inp_images = data['images'][:,:3].to(args.device)
            ...
            generated_points = decoder(img_features)
    ...
    def load(self, resume_name):
        print(f"Loading {resume_name} model")
    ...
    def load_decoder(self, resume_name):
        print(f"Loading {resume_name} model")
        checkpoint = torch.load(os.path.join(args.logdir, resume_name, "recent.ckpt"))
        decoder.load_state_dict(checkpoint['decoder'])
...
trainer = Engine(print_every=args.print_every, resume_name=args.resume, resume_decoder=args.expid_decoder)
if args.expid_decoder:
    print("Train only resnet encoder")
...
print("Train resnet + decoder")
optimizer = optim.Adam(list(encoder.parameters()) + list(decoder.parameters()), lr=args.learning_rate)
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[200, 400], gamma=0.1)

for epoch in range(trainer.cur_epoch, 2 * args.epochs):
    trainer.train()
    if epoch % args.val_every == 0:
        trainer.validate()
    trainer.save()
