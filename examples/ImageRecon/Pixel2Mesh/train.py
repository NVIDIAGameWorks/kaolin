...
points_set = kal.datasets.ShapeNet_Points(root='../../datasets/', cache_dir='cache/', categories=args.categories,
...
valid_set = kal.datasets.ShapeNet_Combination([points_set_valid, images_set_valid], root='../../datasets/')
    ...
    def train(self):
        ...
        for i, data in enumerate(tqdm(dataloader_train), 0):
            ...
            full_vert_features = torch.cat(
                (meshes['init'][1].vertices, projected_image_features, future_features), dim=1)
            ...
            projected_image_features = pooling(img_features, pool_indices)
            full_vert_features = torch.cat(
                (meshes['init'][2].vertices, projected_image_features, future_features), dim=1)
            ...
            if i % args.print_every == 0:
                ...
                message = message + \
                    f'Lap: {(lap_loss.item()):3.3f}, Edge: {(edge_loss.item()):3.3f}, Norm: {(norm_loss.item()):3.3f}'
