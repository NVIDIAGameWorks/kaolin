...
class Floor(utils.ForceWrapper):
    ...
    def _hessian(self, x):
        ...
        if self.cached_hess == None:
            self.cached_hess = torch.zeros(x.shape[0], x.shape[0], x.shape[1],
...
class Boundary(utils.ForceWrapper):
    ...
    def _hessian(self, x):
        ...
        self.cached_hess[self.pinned_indices, self.pinned_indices] = 2 * \
            torch.eye(x.shape[1], device=x.device, dtype=x.dtype)
