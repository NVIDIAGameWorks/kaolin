...
class SimplicitsParticleNewtonShapeSoftContact:
    ...
    def energy(self, dx, x0, coeff, energy=None):
        ...
        Args:
            dx (wp.array): Current CP displacements with the current dofs; dtype=wp.vec3, size :math:`(\text{num_pts},)`.
            x0 (wp.array): Rest contact point positions; dtype=wp.vec3, size :math:`(\text{num_pts},)`.
    ...
    def gradient(self, dx, x0, coeff, gradients):
        ...
        Args:
            ...
            gradients (wp.array): Output gradient; dtype=wp.vec3, size :math:`(\text{num_pts},)`.

        Returns:
            (wp.array(dtype=wp.vec3)): Optional output gradient of size :math:`(\text{num_pts},)`.
        """
    ...
    def hessian(self, dx, x0, coeff):
        ...
        Args:
            dx (wp.array): Current integration-point displacements; dtype=wp.vec3, size :math:`(\text{num_pts},)`.
            x0 (wp.array): Rest integration-point positions; dtype=wp.vec3, size :math:`(\text{num_pts},)`.
            coeff (float): Scaling coefficient applied to the Hessian (e.g. time-step factor).

        Returns:
            (wp.array(dtype=wp.mat33)): Hessian blocks of size :math:`(\text{num_pts},)` (same
