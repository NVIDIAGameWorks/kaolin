...
template<typename scalar_t, typename vector_t>
__global__  void TriangleDistanceKernel(
...
template<typename scalar_t, typename vector_t>
__global__ void TriangleDistanceBackwardKernel(
...
void triangle_distance_cuda_forward(
  ...
  DISPATCH_NUM_TYPES(points.scalar_type(), scalar_t, "triangle_distance", [&] {
  using vector_t = ScalarTypeToVec3Type<scalar_t>::type;
  TriangleDistanceKernel<scalar_t, vector_t><<<dim3(32,16,1),512>>>(points.data_ptr<scalar_t>(), verts_1.data_ptr<scalar_t>(),
                                                                    verts_2.data_ptr<scalar_t>(), verts_3.data_ptr<scalar_t>(),
                                                                    points.size(0), verts_1.size(0),
                                                                    dist1.data_ptr<scalar_t>(), idx1.data_ptr<int64_t>(), type1.data_ptr<int>());
});
}
...
DISPATCH_NUM_TYPES(points.scalar_type(), scalar_t, "triangle_distance", [&] {
  ...
  TriangleDistanceBackwardKernel<scalar_t, vector_t><<<num_blocks, BLOCK_SIZE>>>(grad_output.data_ptr<scalar_t>(), points.data_ptr<scalar_t>(),
                                                                                 verts_1.data_ptr<scalar_t>(), verts_2.data_ptr<scalar_t>(),
                                                                                 verts_3.data_ptr<scalar_t>(), points.size(0), verts_1.size(0),
