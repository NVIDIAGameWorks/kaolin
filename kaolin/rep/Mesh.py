...
class Mesh():
    ...
    @_composedecorator(classmethod, abstractmethod)
    def from_obj_old(self, filename: str, with_vt: bool = False,
                     enable_adjacency: bool = False, texture_res=4):
    ...
    @staticmethod
    def homogenize_faces(faces):
        is_homogeneous = all(len(f) == len(faces[0]) for f in faces)
        if not is_homogeneous:
            new_faces = []
            # homogenize by converting quads to triangles
            for face in faces:
                if len(face) == 4:
                    new_faces.append([face[0], face[1], face[3]])
                    new_faces.append([face[1], face[2], face[3]])
                else:
                    new_faces.append(face)
