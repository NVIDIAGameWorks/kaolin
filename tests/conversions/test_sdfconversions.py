...
def test_sdf_to_pointcloud():
    ...
    points = kal.conversions.sdf_to_pointcloud(
        sdf, bbox_center=0., resolution=10, bbox_dim=1, num_points=10000)
    ...
    points = kal.conversions.sdf_to_pointcloud(
        sdf, bbox_center=0., resolution=32, bbox_dim=1, num_points=10000)
...
def test_sdf_to_trianglemesh():
    ...
    verts, faces = kal.conversions.sdf_to_trianglemesh(
        sdf, bbox_center=0., resolution=10, bbox_dim=1)

    assert kal.rep.SDF._length(verts).mean() <= .6
    assert kal.rep.SDF._length(verts).mean() >= .4

    verts, faces = kal.conversions.sdf_to_trianglemesh(
        sdf, bbox_center=0., resolution=32, bbox_dim=1)
...
def test_sdf_to_voxelgrid():
    ...
    sdf = kal.rep.SDF.sphere()
    voxels = kal.conversions.sdf_to_voxelgrid(
        sdf, bbox_center=0., resolution=10, bbox_dim=1)
    assert voxels[20, 20, 20] == -0.5
    assert voxels[20, 20, 0] == 0.

    voxels = kal.conversions.sdf_to_voxelgrid(
        sdf, bbox_center=0., resolution=32, bbox_dim=1)

    assert voxels[64, 64, 64] == -0.5
    assert voxels[64, 64, 0] == 0.
